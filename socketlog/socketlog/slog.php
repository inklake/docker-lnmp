...
class Slog {
	...
	public static function trace($msg, $trace_level = 1, $css = '') {
		...
		for ($i = 0; $i < $max; $i++) {
			...
			// 这里对源文件有修改 防止传入对象msg导致输出格式错误 源文件 当fun 等于 slog  输入数据对象 会导致错误
			// if ($fun != 'slog' && !empty($trace['args'])) {
			if (!empty($trace['args'])) {
	...
	public static function error_handler($errno, $errstr, $errfile, $errline) {
		switch ($errno) {
			...
			case E_USER_DEPRECATED:$severity = 'E_USER_DEPRECATED';
				break;
			case E_ERROR:$severity = 'E_ERR';
				break;
			case E_PARSE:$severity = 'E_PARSE';
				break;
			case E_CORE_ERROR:$severity = 'E_CORE_ERROR';
				break;
			case E_COMPILE_ERROR:$severity = 'E_COMPILE_ERROR';
				break;
			case E_USER_ERROR:$severity = 'E_USER_ERROR';
				break;
			default:$severity = 'E_UNKNOWN_ERROR_' . $errno;
				break;
		}
	...
	public function record($type, $msg = '', $css = '') {
		...
		self::$logs[] = array(
			...
			// 'msg'  => json_decode(json_encode($msg), true), // 解决复杂对象输出格式错误的问题 但是效率底下
			'msg'  => json_encode($msg, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), // 解决复杂对象输出格式错误的问题 但是效率底下
			// 'msg'  => json_encode($msg), // 解决复杂对象输出格式错误的问题 但是效率底下
	...
	protected static function sendToClient($tabid, $client_id, $logs, $force_client_id) {
		...
		if (empty($msg)) {
			$logs['logs'] = array(array('type' => 'log', 'msg' => 'socketlog 错误: json_last_error: ' . json_last_error()));
